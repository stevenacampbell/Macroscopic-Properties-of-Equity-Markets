...
# LOOP OVER YEARS AND FREQUENCY
years<-seq(1962,2023,1)
freqs<-c(1,2,5,10,25,50,125,500)

#p value and transaction cost
pval<-0
tc<-0.0025
...
foreach(i=seq(1,length(years),1), .packages=c("readr")) %dopar% {
  for(j in seq(1,length(freqs),1)){
    
    print(paste0("YEAR: ",years[i],", freq=",freqs[j]))
    ...
    freq<-freqs[j] #Trading frequency
