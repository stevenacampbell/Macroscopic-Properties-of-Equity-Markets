...
caps <- read_csv("Test_Data/BACKTESTING_test_caps.csv", 
                 col_types =  cols(.default = "d"))
caps <- as.data.frame(caps)

# Import saved CRSP security returns

rets <- read_csv("Test_Data/BACKTESTING_test_rets.csv", 
                 col_types =  cols(.default = "d"))
rets <- as.data.frame(rets)

# Import saved CRSP delisting flag

dlflg <- read_csv("Test_Data/BACKTESTING_test_dlflg.csv", 
                  col_types =  cols(.default = col_logical()))
dlflg <- as.data.frame(dlflg)

# Import dates

dates <-  read_csv("Test_Data/BACKTESTING_test_dates.csv", 
                   col_types = cols(dates = col_date(format = "%Y-%m-%d")))
dates <- as.data.frame(dates)[[1]]

# Import permnos

permnos <-  read_csv("Test_Data/BACKTESTING_test_permnos.csv",
